...
<head>
    @php
        ...
        $seoKeywords = $job->title . ', ' . $tenantModel->name . ', ' . $job->department->name . ', ' . ($job->location?->name ?? $job->globalLocation?->name ?? $job->city?->formatted_location ?? 'Location') . ', careers, jobs, hiring, employment, TalentLit';
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <a href="{{ route('careers.index', ['tenant' => $tenantModel->slug]) }}" class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 shadow-lg">
                            View All Positions
                        </a>
                    </div>
                </div>
                ...
                <div class="text-center">
                    ...
                    <div class="flex flex-wrap items-center justify-center gap-6 text-indigo-100 mb-8">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <span>{{ $job->location?->name ?? $job->globalLocation?->name ?? $job->city?->formatted_location ?? 'No location specified' }}</span>
                        </div>
                        <div class="flex items-center space-x-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        <a href="{{ route('careers.apply.create', ['tenant' => $tenantModel->slug, 'job' => $job->slug]) }}" 
                        ...
                        <a href="{{ route('careers.index', ['tenant' => $tenantModel->slug]) }}" 
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-300">
                            <div class="flex items-center mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600">{{ $job->location?->name ?? $job->globalLocation?->name ?? $job->city?->formatted_location ?? 'No location specified' }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-300">
                            <div class="flex items-center mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600">{{ ucfirst(str_replace('_', ' ', $job->employment_type)) }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-300">
                            <div class="flex items-center mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600">{{ $job->openings_count }} position{{ $job->openings_count > 1 ? 's' : '' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-all duration-300">
                                        <div class="flex items-center mb-4">
                                            <div class="w-10 h-10 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg flex items-center justify-center mr-4">
                                                <span class="text-sm font-bold text-white">{{ $stage->sort_order }}</span>
                                            </div>
                                            <div>
