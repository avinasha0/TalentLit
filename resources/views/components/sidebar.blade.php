...
@click.away="if (window.innerWidth < 1024) $store.sidebar.toggle()">
    ...
    <a href="{{ tenantRoute('tenant.dashboard', $tenant->slug) }}"
            ...
            <!-- My Tasks -->
            <a href="{{ tenantRoute('tenant.tasks.my', $tenant->slug) }}"
               class="flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-colors duration-200 {{ $isTasks ? 'bg-purple-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
                ...
                <div x-show="employeeOnboardingOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" class="ml-8 mt-2 space-y-1">
                    ...
                    @endif
                    <a href="{{ tenantRoute('tenant.employee-onboarding.pre-onboarding', $tenant->slug) }}"
                       class="flex items-center px-3 py-2 text-sm text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ (str_starts_with($currentRoute, 'tenant.employee-onboarding.pre-onboarding') || str_starts_with($currentRoute, 'subdomain.employee-onboarding.pre-onboarding')) ? 'bg-gray-700 text-white' : '' }}">
                        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
