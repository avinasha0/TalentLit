        ...
        <div class="flex items-center justify-between h-16 px-4 border-b border-gray-700">
            <div class="flex items-center space-x-3">
                @if($branding && $branding->logo_path)
                    <img src="{{ Storage::url($branding->logo_path) }}" alt="{{ $tenant->name }}" class="h-8 w-8 rounded">
                @else
                ...
                @endif
                <div>
                    <h1 class="text-lg font-semibold text-white">{{ $tenant->name ?? 'TalentLit' }}</h1>
    ...
    <a href="{{ route('tenant.candidates.index', $tenant->slug) }}"
            ...
            <div>
                ...
                <div x-show="departmentsOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" class="ml-8 mt-2 space-y-1">
                    <a href="{{ route('tenant.departments.index', $tenant->slug) }}"
                       class="flex items-center px-3 py-2 text-sm text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ str_starts_with($currentRoute, 'tenant.departments.index') ? 'bg-gray-700 text-white' : '' }}">
                        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @customCan('edit_jobs', $tenant)
                    <a href="{{ route('tenant.departments.create', $tenant->slug) }}" 
                       class="flex items-center px-3 py-2 text-sm text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ $currentRoute === 'tenant.departments.create' ? 'bg-gray-700 text-white' : '' }}">
                        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
                        Add Department
            ...
            <a href="{{ route('tenant.interviews.index', $tenant->slug) }}" 
               class="flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-colors duration-200 {{ $isInterviews ? 'bg-purple-600 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
