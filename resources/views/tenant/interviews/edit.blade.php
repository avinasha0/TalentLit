            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                        ...
                        <p class="text-sm text-gray-600">{{ $interview->candidate->first_name }} {{ $interview->candidate->last_name }}</p>
                        <p class="text-sm text-gray-500">{{ $interview->candidate->primary_email }}</p>
                    </div>

                    <form method="POST" action="{{ route('tenant.interviews.update', ['tenant' => $tenant, 'interview' => $interview]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="job_id" id="job_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    ...
                                    @foreach($jobs as $job)
                                        <option value="{{ $job->id }}" {{ old('job_id', $interview->job_id) == $job->id ? 'selected' : '' }}>
                                            {{ $job->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="duration_minutes" id="duration_minutes" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                                    ...
                                    <option value="60" {{ old('duration_minutes', $interview->duration_minutes) == '60' ? 'selected' : '' }}>1 hour</option>
                                    <option value="90" {{ old('duration_minutes', $interview->duration_minutes) == '90' ? 'selected' : '' }}>1.5 hours</option>
                            ...
                            <div>
                                ...
                                <select name="mode" id="mode" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                                    ...
                                    <option value="onsite" {{ old('mode', $interview->mode) == 'onsite' ? 'selected' : '' }}>Onsite</option>
                                    <option value="remote" {{ old('mode', $interview->mode) == 'remote' ? 'selected' : '' }}>Remote</option>
                                    <option value="phone" {{ old('mode', $interview->mode) == 'phone' ? 'selected' : '' }}>Phone</option>
                                </select>
                                @error('mode')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div id="location-field" class="{{ $interview->mode === 'onsite' ? '' : 'hidden' }}">
                            ...
                            <div id="meeting-link-field" class="{{ $interview->mode === 'remote' ? '' : 'hidden' }}">
                                    ...
                                    @foreach($users as $user)
                                        <label class="flex items-center">
                                            <input type="checkbox" 
                                                   name="panelists[]" 
                                                   value="{{ $user->id }}"
                                                   {{ in_array($user->id, old('panelists', $interview->panelists->pluck('id')->toArray())) ? 'checked' : '' }}
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="notes" 
                                          ...
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $interview->notes) }}</textarea>
                                @error('notes')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mt-6 flex justify-end space-x-3">
                            <a href="{{ route('tenant.interviews.show', ['tenant' => $tenant, 'interview' => $interview]) }}" 
